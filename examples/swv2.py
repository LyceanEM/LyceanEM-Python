...
for angle_inc in tqdm(range(len(angle_values))):
    ...
    if (single_source):
        ...
        z=np.full((1,1),0.,dtype=np.float32)
        ...
        normals[0,2]=1.0
        transmitting_antenna_surface_coords.normals=o3d.utility.Vector3dVector(normals)
        transmit_horn_structure,_=TL.meshedHorn(58e-3,58e-3,128e-3,2e-3,0.21,aperture_resolution)
    else:
        #horn spec with x_width, y_width, length, edge width, flare angle, and aperture resolution
        transmit_horn_structure,transmitting_antenna_surface_coords=TL.meshedHorn(58e-3,58e-3,128e-3,2e-3,0.21,aperture_resolution)
    ...
    if (single_sink):
        ...
        receive_horn_structure,_=TL.meshedHorn(58e-3,58e-3,128e-3,2e-3,0.21,aperture_resolution)
    else:
        receive_horn_structure,recieving_antenna_surface_coords=TL.meshedHorn(58e-3,58e-3,128e-3,2e-3,0.21,aperture_resolution)
    ...
    transmitting_antenna_points=len(np.asarray(transmitting_antenna_surface_coords.points))
    ...
    scatter_cloud=scatter_points.rotate(o3d.geometry.TriangleMesh.get_rotation_matrix_from_xyz(rotation_vector1),center=False)
    reflectorplate.rotate(o3d.geometry.TriangleMesh.get_rotation_matrix_from_xyz(rotation_vector1),center=False)
    ...
    scatter_cloud.rotate(o3d.geometry.TriangleMesh.get_rotation_matrix_from_xyz(rotation_vector),center=False)
    reflectorplate.rotate(o3d.geometry.TriangleMesh.get_rotation_matrix_from_xyz(rotation_vector),center=False)
    ...
    temp_environment=np.append(np.append(RF.convertTriangles(reflectorplate),RF.convertTriangles(transmit_horn_structure),axis=0),RF.convertTriangles(receive_horn_structure),axis=0)
    ...
    if (show_scene):
        ...
        o3d.visualization.draw_geometries([source_points,sink_points,scatter_cloud,reflectorplate,mesh_frame,receive_horn_structure,transmit_horn_structure])
    ...
    #                                                                                                scattering=1)
    Ex,Ey,Ez=FD.calculate_scattering(aperture_coords=transmitting_antenna_surface_coords,
                                     ...
                                     wavelength=wavelength,
                                     scattering=1)
    isoresponse[angle_inc]=FD.calculate_scattering_isotropic(aperture_coords=transmitting_antenna_surface_coords,
                                                  sink_coords=recieving_antenna_surface_coords,
                                                  antenna_solid=solids,
                                                  scatter_points=scatter_cloud,
                                                  wavelength=wavelength,
...
normalised_max3=np.max(np.array([np.max(20*np.log10(np.abs(responsex2))),np.max(20*np.log10(np.abs(responsey2))),np.max(20*np.log10(np.abs(responsez2)))]))
