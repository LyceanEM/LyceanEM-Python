...
py::array_t<std::complex<float>> calculate_scattering_tiles(py::array_t<float> source, py::array_t<float> end, 
    ...
    raycast_wrapper_tiles(&source_ptr[source_index], end_ptr, source_size, end_size,d_tri_vertex, d_binned_triangles, d_bin_count
            ,make_int2(n_cellsx,n_cellsy) , make_float2(xmin,xmax), make_float2(ymin,tile_size), make_float2(zmin,tile_size),points, wave_length,scattering_network_ptr,make_float2(alpha,beta));
