...
__device__ __inline__ complex_float3 em_wave2(int ray_wave, const float4& ray, const PointData& origin, const PointData& end,const float wave_length, const float alpha, const float beta) {
    ...
    G *= (expf(-alpha*ray.w) *(1/pow((pow(ray.w,2.5)+1),0.4)));
    cuFloatComplex dG;
    dG.x = -alpha - (1/pow((pow(ray.w,2.5)+1),0.4));
    dG.y = -beta;
    dG  = cuCmulf(dG,G);

    cuFloatComplex loss = front * dG * dot(origin.normal,make_float3(ray.x,ray.y,ray.z));
...
__global__ void raycast2(float3 *source, float3 *end, float4 *ray, int source_num, int end_num, int flag, float3 *tri_vertex,int3 *binned_triangles, int2* tri_num_in_bin, int ray_num,
     int2 *ray_index, int x_offset, int y_offset, int2 num_bins, float2 x_top_bottom, float2 y_range, float2 z_range,PointData* points, float wave_length, complex_float3* scattering_network, const float alpha, const float beta)
{
...
void raycast_wrapper2 (float *source, float *end, int source_num, int end_num, float3 *d_tri_vertex,int3 *d_binned_triangles, int2* d_tri_num_per_bin, int2 num_bins, float2 x_top_bottom, float2 y_range, float2 z_range,
    PointData* points, float wave_length, complex_float3* h_scattering_network, const float alpha, const float beta)
{
