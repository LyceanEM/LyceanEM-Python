...
py::array_t<std::complex<float>> em_test(py::array_t<float> source, py::array_t<float> end, py::array_t<float> scatter, py::array_t<int> index, py::array_t<float> ex_real, py::array_t<float> ex_imag, py::array_t<float> ey_real,
...

{
    ...
    int* index_ptr = (int*) index.request().ptr;
    int points_size = source.shape(0) + end.shape(0) + scatter.shape(0);
    ...
    float3 source_vec = make_float3(source_ptr[0], source_ptr[1], source_ptr[2]);
    float3 end_vec = make_float3(end_ptr[0], end_ptr[1], end_ptr[2]);
    float3 scatter_vec = make_float3(scatter_ptr[0], scatter_ptr[1], scatter_ptr[2]);

    float4 ray[3];
    float3 direction = normalize(end_vec-source_vec);
    ray[0] = make_float4(direction.x, direction.y, direction.z, length(end_vec-source_vec));
    direction = normalize(scatter_vec-source_vec);
    ray[1] = make_float4(direction.x, direction.y, direction.z, length(scatter_vec-source_vec));
    direction = normalize(end_vec-scatter_vec);
    ray[2] = make_float4(direction.x, direction.y, direction.z, length(end_vec-scatter_vec));
    ...
    frequency_wrapper(0, d_ray, points, d_ray_index, wave_vector, 1, 1, 1, scattering_network);
    gpuErrchk( cudaGetLastError() );
    cudaFree(d_ray);
    ...
    for (int i = 0; i < 1; i++){
        scattering_network_data[i] = std::complex<float>(scattering_network[i].x.x, scattering_network[i].x.y);
        scattering_network_data[i+1] = std::complex<float>(scattering_network[i].y.x, scattering_network[i].y.y);
        scattering_network_data[i+2] = std::complex<float>(scattering_network[i].z.x, scattering_network[i].z.y);
    }
