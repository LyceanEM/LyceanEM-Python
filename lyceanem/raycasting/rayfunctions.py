...
def hit(ray,triangle):
    ...
    if (intersect_distance>(2*EPSILON)) and (intersect_distance<(ray.dist-(2*EPSILON))):
    ...
    if (cu_ray_num<ray_index.shape[0]):
        ...
        for i in range(ray_index.shape[1]-1):
            ...
            if (ray_index[cu_ray_num,i+1]!=0):
                ...
                ray[0]['oy']=point1['py']
                ray[0]['oz']=point1['pz']
                normconst=math.sqrt((point2['px']-point1['px'])**2+(point2['py']-point1['py'])**2+(point2['pz']-point1['pz'])**2)
...
def kernel1Dv3(rays,environment):
    ...
    if (cu_ray_num<rays.shape[0]):                    #           threadIdx.y + ( blockIdx.y * blockDim.y )
        #margin=1e-5
        #dist_min=math.inf
        #intersect_check=False
        max_tri_num=len(environment)
...
def integratedRaycaster(ray_index,scattering_points,environment_local):
    ...
    integratedkernal[grids, threads](d_ray_index,d_point_information,d_environment,d_ray_flag)
    #cuda.profile_stop()
...
def visiblespace(source_coords,source_normals,environment,vertex_area=0,az_range=np.linspace(-180.0,180.0,19),elev_range=np.linspace(-90.0,90.0,19),shell_range=0.5):
    ...
    sinks[:,0],sinks[:,1],sinks[:,2]=azeltocart(np.ravel(azaz),np.ravel(elel),1e9)
    sinknum=len(sinks)
    #
    ...
    hit_index,_=workchunkingv2(source_coords,sinks,np.empty((0,3),dtype=np.float32),environment,1)
    unified_model=np.append(source_coords.astype(np.float32),sinks.astype(np.float32),axis=0)
    ...
    shell_coords[:,0],shell_coords[:,1],shell_coords[:,2]=azeltocart(np.ravel(azaz),np.ravel(elel),shell_range)
    maxarea=np.nanmax(np.nanmax(visible_patterns))
    resultant_pcd=patterntocloud(np.reshape(visible_patterns,(len(az_range)*len(elev_range),1)),shell_coords,maxarea)
...
def angle_pop(hit_directions,source_normals,angles):
    ...
    for angle_inc in prange(len(angles)):
        angles[angle_inc]=angle(hit_directions[angle_inc,:],source_normals[angle_inc,:])
...
def patterncreator(az_range,elev_range,source_coords,pointingindex,hit_index):
    ...
    visible_patterns=np.empty((len(az_range),len(elev_range),len(source_coords)),dtype=np.int32)
    visible_patterns[:,:,:]=0
    for n in range(len(az_range)):
        for m in range(len(elev_range)):
            for element in range(len(source_coords)):
                if (np.any(np.all(np.asarray([hit_index[:,0]==element,hit_index[:,1]==pointingindex[n,m]]),axis=0))):
                    visible_patterns[n,m,element]=1
    return visible_patterns


def quickpatterncreator(az_range,elev_range,source_coords,angles,vertex_area,hit_index):
    #creates visibility pattern from azimuth and elevation ranges
    visible_patterns=np.empty((len(az_range),len(elev_range)),dtype=np.float32)
    visible_patterns[:,:]=0
    pointingindex=np.reshape(np.arange(0,len(az_range)*len(elev_range)),(len(az_range),len(elev_range)))
    sourcenum=len(source_coords)
    if (vertex_area==0):
        for n in range(len(az_range)):
            for m in range(len(elev_range)):
                visible_patterns[n,m]=np.sum(hit_index[:,1]-sourcenum-1==pointingindex[n,m])
    else:
        portion=vertex_area*np.cos(angles)
        portion[portion<0]=0
        for n in range(len(az_range)):
            for m in range(len(elev_range)):
                visible_patterns[n,m]=np.sum(portion[hit_index[:,1]-sourcenum-1==pointingindex[n,m]])
...
def PathLength(partial_network,path_length):
    ...
    if (partial_network.shape[1]==6):
        for i in range(partial_network.shape[0]):
            path_length[i]=np.sqrt((partial_network[i,3]-partial_network[i,0])**2+(partial_network[i,4]-partial_network[i,1])**2+(partial_network[i,5]-partial_network[i,2])**2)
    elif (partial_network.shape[1]==9):
        for i in range(partial_network.shape[0]):
            path_length[i]=np.sqrt((partial_network[i,3]-partial_network[i,0])**2+(partial_network[i,4]-partial_network[i,1])**2+(partial_network[i,5]-partial_network[i,2])**2)+np.sqrt((partial_network[i,6]-partial_network[i,3])**2+(partial_network[i,7]-partial_network[i,4])**2+(partial_network[i,8]-partial_network[i,5])**2)
    elif (partial_network.shape[1]==12):
        for i in range(partial_network.shape[0]):
            path_length[i]=np.sqrt((partial_network[i,3]-partial_network[i,0])**2+(partial_network[i,4]-partial_network[i,1])**2+(partial_network[i,5]-partial_network[i,2])**2)+np.sqrt((partial_network[i,6]-partial_network[i,3])**2+(partial_network[i,7]-partial_network[i,4])**2+(partial_network[i,8]-partial_network[i,5])**2)+np.sqrt((partial_network[i,9]-partial_network[i,6])**2+(partial_network[i,10]-partial_network[i,7])**2+(partial_network[i,11]-partial_network[i,8])**2)


#@guvectorize([(int32[:,:], float32[:,:], float32[:,:], float32[:,:], float32, complex64[:,:])], '(m,n)->(m,n)',target='parallel')
@njit(parallel=True,nogil=True,cache=True)
def ScatteringNetworkGenerator(network_index,scattering_points,scattering_normals,scattering_weights,point_information,scattering_coefficient,wavelength,local_channel):
    """
    for each row in the partial_network, calculate the distance travelled from the origin to the sink point
    #
    Parameters
    ...
    if (network_index.shape[1]==2):
        for i in prange(network_index.shape[0]):
            local_channel[i,:]=EM.losChannel(scattering_points[network_index[i,0]-1,:],
        ...
        for i in prange(network_index.shape[0]):
            local_channel[i,:]=EM.losplus1Channel(scattering_points[network_index[i,0]-1,:],
                                                  scattering_normals[network_index[i,0]-1,:],
                                                  scattering_weights[network_index[i,0]-1,:],
                                                  point_information[network_index[i,0]-1],
                                                  scattering_points[network_index[i,1]-1,:],
        ...
        for i in prange(network_index.shape[0]):
            local_channel[i,:]=EM.losplus2Channel(scattering_points[network_index[i,0]-1,:],
                                                  scattering_normals[network_index[i,0]-1,:],
                                                  scattering_weights[network_index[i,0]-1,:],
                                                  point_information[network_index[i,0]-1],
                                                  scattering_points[network_index[i,1]-1,:],
...
@njit(parallel=True,nogil=True,cache=True)
def ScatteringNetworkGeneratorTime(network_index,scattering_points,scattering_normals,scattering_weights,point_information,wavelength,local_channel,times):
    """
    for each row in the partial_network, calculate the distance travelled from the origin to the sink point
    #
    Parameters
    ...
    if (network_index.shape[1]==2):
        for i in prange(network_index.shape[0]):
            local_channel[i,:],times[i]=EM.losChannelv2(scattering_points[network_index[i,0]-1,:],
                                             scattering_normals[network_index[i,0]-1,:],
                                             scattering_weights[network_index[i,0]-1,:],
                                             point_information[network_index[i,0]-1],
                                             scattering_points[network_index[i,1]-1,:],
        ...
        for i in prange(network_index.shape[0]):
            local_channel[i,:],times[i]=EM.losplus1Channelv2(scattering_points[network_index[i,0]-1,:],
                                                  scattering_normals[network_index[i,0]-1,:],
                                                  scattering_weights[network_index[i,0]-1,:],
                                                  point_information[network_index[i,0]-1],
                                                  scattering_points[network_index[i,1]-1,:],
        ...
        for i in prange(network_index.shape[0]):
            local_channel[i,:],times[i]=EM.losplus2Channelv2(scattering_points[network_index[i,0]-1,:],
                                                  scattering_normals[network_index[i,0]-1,:],
                                                  scattering_weights[network_index[i,0]-1,:],
                                                  point_information[network_index[i,0]-1],
                                                  scattering_points[network_index[i,1]-1,:],
...
@njit(parallel=True)
def ScatteringNetworkGeneratorv2(network_index,scattering_points,scattering_normals,scattering_weights,point_information,wavelength,local_channel):
    """
    for each row in the partial_network, calculate the distance travelled from the origin to the sink point
    #
    Parameters
    ...
    """
    if (network_index.shape[1]==2):
        local_channel=EM.losChannel(scattering_points[network_index[0]-1],
        ...
        local_channel=EM.losplus1Channel(scattering_points[network_index[0]-1],
                                    scattering_normals[network_index[0]-1],
                                    scattering_weights[network_index[0]-1],
                                    point_information[network_index[0]-1],
                                    scattering_points[network_index[1]-1],
        ...
        local_channel=EM.losplus2Channel(scattering_points[network_index[0]-1],
                                    scattering_normals[network_index[0]-1],
                                    scattering_weights[network_index[0]-1],
                                    point_information[network_index[0]-1],
                                    scattering_points[network_index[1]-1],
...
def PoyntingVector(partial_network):
    ...
    for i in range(partial_network.shape[0]):
        ...
        pointing_mags[i]=np.sqrt(pointing_vectors[i,0]**2+pointing_vectors[i,1]**2+pointing_vectors[i,2]**2)
...
def CalculatePoyntingVectors(total_network,
    ...
    if len(total_network)==0:
        ...
        for idx in range(scatterdepth):
            if(idx==(scatterdepth-1)):
                ...
                if scatterdepth==1:
                     ...
                     start_row=np.max(np.where((np.isnan(total_network[:,6+((idx-1)*3)]))))+1
                     end_row=len(total_network[:,1])-1

                end_col=(idx*3)+6
                #engine here
                paths=np.append(paths,PathLength(total_network[start_row:end_row+1,:end_col]),axis=0)
                total_pointing_vectors=np.append(total_pointing_vectors,AngleofArrivalVectors(total_network[start_row:end_row+1,:end_col]),axis=0)
                ...
                #indexing
                start_row=np.max(np.where((np.isnan(total_network[:,3+(idx*3)]))))+1
                end_row=np.max(np.where((np.isnan(total_network[:,6+((idx)*3)]))))
                end_col=(idx*3)+6
                #engine here
                paths=np.append(paths,PathLength(total_network[start_row:end_row+1,:end_col]),axis=0)
        ...
        for sink_index in range(sinknum):
                ...
                if impulse:
                    impulse_response=ImpulseStack(impulse_response,ray_components,sink_index,sink_indexing,travel_times,time_bins)
                ...
                if aoa:
                    angle_response=angleofarrivalStack(angle_response,ray_components,sink_index,sink_indexing,angular_dist,az_bins)
...
@njit(parallel=True)
def ImpulseStack(impulse_response,ray_components,sink_index,sink_indexing,travel_times,time_bins):
...
@njit(parallel=True)
def angleofarrivalStack(angle_response,ray_components,sink_index,sink_indexing,angular_dist,az_bins):
...
def Angular_distance(center_vector,pointing_vectors):
    if len(center_vector)==1:
        ...
        angle_lists=np.zeros((len(pointing_vectors),1),dtype=np.float32)
        for i in range(len(pointing_vectors)):
            angle_lists[i]=np.arccos(np.dot(center_vector[i,:],pointing_vectors[i,:]))
            #angle_lists[i]=np.arctan2(pointing_vectors[i,1],pointing_vectors[i,0])
...
#@jit
def VectorNetworkProcessEM(sourcenum,sinknum,unified_points,unified_normals,unified_weights,point_information,scattering_index,scattering_coefficient,wavelength):
    """
    ...
    if scattering_index.shape[0]==0:
        ...
        for idx in range(scatterdepth):
            if(idx==(scatterdepth-1)):
                ray_analysis_index=scattering_index[~np.equal(scattering_index[:,idx+1],0),:]
                ray_components=np.zeros((ray_analysis_index.shape[0],3),dtype=np.complex64)
                ray_components=ScatteringNetworkGenerator(ray_analysis_index,unified_points,unified_normals,unified_weights,point_information,scattering_coefficient,wavelength,ray_components)
                #path_loss=((1j*wave_vector)/(4*np.pi))*(np.exp(paths*wave_vector*1j)/paths)
                depth_slice=ray_analysis_index[:,[0,-1]]

            elif idx==0:
                ray_analysis_index=scattering_index[np.equal(scattering_index[:,idx+2],0),0:idx+2]
                ray_components=np.zeros((ray_analysis_index.shape[0],3),dtype=np.complex64)
                ray_components=ScatteringNetworkGenerator(ray_analysis_index,unified_points,unified_normals,unified_weights,point_information,scattering_coefficient,wavelength,ray_components)
                #path_loss=((1j*wave_vector)/(4*np.pi))*(np.exp(paths*wave_vector*1j)/paths)
                ...
                ray_analysis_index=scattering_index[~np.equal(scattering_index[:,idx+1],0),0:idx+2]
                ray_components=np.zeros((ray_analysis_index.shape[0],3),dtype=np.complex64)
                ray_components=ScatteringNetworkGenerator(ray_analysis_index,unified_points,unified_normals,unified_weights,point_information,scattering_coefficient,wavelength,ray_components)
                #path_loss=((1j*wave_vector)/(4*np.pi))*(np.exp(paths*wave_vector*1j)/paths)
                depth_slice=ray_analysis_index[:,[0,-1]]
            ...
            scatter_map=scatter_net_sortEMtest(sourcenum,sinknum,scatter_map,depth_slice,ray_components,idx)

    return scatter_map

def VectorNetworkProcessTimeEM(sourcenum,sinknum,unified_points,unified_normals,unified_weights,point_information,scattering_index,wavelength):
    """
    A more efficienct version of BaseNetworkProcess, using the vectorised path length function to process the total_network array more efficienctly.
    #
    Parameters
    ...
    if scattering_index.shape[0]==0:
        ...
        wave_vector=(2*np.pi)/wavelength
        scatterdepth=scattering_index.shape[1]-1
        scatter_map=np.zeros((sourcenum,sinknum,3,len(time_index),scatterdepth),dtype='complex')
        for idx in range(scatterdepth):
            if(idx==(scatterdepth-1)):
                ray_analysis_index=scattering_index[~np.equal(scattering_index[:,idx+1],0),:]
                ray_components=np.zeros((ray_analysis_index.shape[0],3),dtype=np.complex64)
                time_components=np.zeros((ray_analysis_index.shape[0]),dtype=np.float32)
                ray_components,time_components=ScatteringNetworkGeneratorTime(ray_analysis_index,unified_points,unified_normals,unified_weights,point_information,wavelength,ray_components,time_components)
                #path_loss=((1j*wave_vector)/(4*np.pi))*(np.exp(paths*wave_vector*1j)/paths)
                depth_slice=ray_analysis_index[:,[0,-1]]
            ...
            scatter_map=scatter_net_sortTimeEM(sourcenum,sinknum,scatter_map,depth_slice,ray_components,time_components,time_index,idx)

    return scatter_map

def VectorNetworkProcessEMv2(sourcenum,sinknum,unified_points,unified_normals,unified_weights,point_information,scattering_index,wavelength):
    """
    A more efficienct version of BaseNetworkProcess, using the vectorised path length function to process the total_network array more efficienctly.
    #
    Parameters
    ...
    if scattering_index.shape[0]==0:
        ...
        ray_components=EM.EMGPUWrapper(scattering_index,point_information,wavelength)
        for idx in range(scatterdepth):
            if(idx==(scatterdepth-1)):
                ray_analysis_index=scattering_index[~np.equal(scattering_index[:,idx+1],0),:]
                temp_components=ray_components[~np.equal(scattering_index[:,idx+1],0),:]
                ...
                ray_analysis_index=scattering_index[np.equal(scattering_index[:,idx+2],0),0:idx+2]
                temp_components=ray_components[~np.equal(scattering_index[:,idx+2],0),:]
                #ray_components=np.zeros((ray_analysis_index.shape[0],3),dtype=np.complex64)
            ...
            scatter_map=scatter_net_sortEM(sourcenum,sinknum,scatter_map,depth_slice,temp_components,idx)

    return scatter_map

#@jit
def VectorNetworkProcessv2(sources,sinks,scattering_points,scattering_index,wavelength):
    """
    A more efficienct version of BaseNetworkProcess, using the vectorised path length function to process the total_network array more efficienctly.
    #
    Parameters
    ...
    if scattering_index.shape[0]==0:
        ...
        unified_model=np.append(np.append(sources.astype(np.float32),sinks.astype(np.float32),axis=0),scattering_points.astype(np.float32),axis=0)
        wave_vector=(2*np.pi)/wavelength
        ...
        for idx in range(scatterdepth):
            if(idx==(scatterdepth-1)):
                ray_analysis_index=scattering_index[~np.equal(scattering_index[:,idx+1],0),:]
                if idx==0:
                    paths=PathLength(np.append(unified_model[ray_analysis_index[:,0],:]-1,unified_model[ray_analysis_index[:,1]-1,:],axis=1))
                elif(idx==1):
                    paths=PathLength(np.append(np.append(unified_model[ray_analysis_index[:,0]-1,:],unified_model[ray_analysis_index[:,1]-1,:],axis=1),unified_model[ray_analysis_index[:,2]-1,:],axis=1))
                elif(idx==2):
                    paths=PathLength(np.append(np.append(np.append(unified_model[ray_analysis_index[:,0]-1,:],unified_model[ray_analysis_index[:,1]-1,:],axis=1),unified_model[ray_analysis_index[:,2]-1,:],axis=1),unified_model[ray_analysis_index[:,3]-1,:],axis=1))
            ...
            scatter_map=scatter_net_sorttest(sourcenum,sinknum,scatter_map,depth_slice,ray_components,idx)
...
def scatter_net_sort(sourcenum,sinknum,scatter_map,depth_slice,ray_components,idx):
    ...
    for sink_index in range(sinknum):
        for source_index in range(sourcenum):
            scatter_map[source_index,sink_index,idx]=np.nansum(ray_components[(depth_slice[:,0]-1==source_index) & (depth_slice[:,1]-sourcenum-1==sink_index)])

    return scatter_map

def scatter_net_sorttest(sourcenum,sinknum,scatter_map,depth_slice,ray_components,idx):
    #simplified scattering sort to create a scattering network using Numbas prange and loop unrolling.
    for sink_index in range(sinknum):
        for source_index in range(sourcenum):
            scatter_map[source_index,sink_index,idx]=np.nansum(ray_components[(depth_slice[:,0]-1==source_index) & (depth_slice[:,1]-sourcenum-1==sink_index)])
...
def scatter_net_sortEMtest(sourcenum,sinknum,scatter_map,depth_slice,ray_components,idx):
    #simplified scattering sort to create a scattering network using Numbas prange and loop unrolling.
    for sink_index in range(sinknum):
        for source_index in range(sourcenum):
            scatter_map[source_index,sink_index,0,idx]=np.nansum(ray_components[(depth_slice[:,0]-1==source_index) & (depth_slice[:,1]-sourcenum-1==sink_index),0])
            scatter_map[source_index,sink_index,1,idx]=np.nansum(ray_components[(depth_slice[:,0]-1==source_index) & (depth_slice[:,1]-sourcenum-1==sink_index),1])
            scatter_map[source_index,sink_index,2,idx]=np.nansum(ray_components[(depth_slice[:,0]-1==source_index) & (depth_slice[:,1]-sourcenum-1==sink_index),2])


    return scatter_map

@njit(parallel=True)
def scatter_net_sortEM(sourcenum,sinknum,scatter_map,depth_slice,ray_components,idx):
    #simplified scattering sort to create a scattering network using Numbas prange and loop unrolling.
    for sink_index in range(sinknum):
        for source_index in range(sourcenum):
            scatter_map[source_index,sink_index,0,idx]=np.sum(ray_components[(depth_slice[:,0]-1==source_index) & (depth_slice[:,1]-sourcenum-1==sink_index),0])
            scatter_map[source_index,sink_index,1,idx]=np.sum(ray_components[(depth_slice[:,0]-1==source_index) & (depth_slice[:,1]-sourcenum-1==sink_index),1])
            scatter_map[source_index,sink_index,2,idx]=np.sum(ray_components[(depth_slice[:,0]-1==source_index) & (depth_slice[:,1]-sourcenum-1==sink_index),2])

    return scatter_map

#@njit(parallel=True)
def scatter_net_sortTimeEM(sourcenum,sinknum,scatter_map,depth_slice,ray_components,time_components,time_index,idx):
    #simplified scattering sort to create a scattering network using Numbas prange and loop unrolling.
    for sink_index in range(sinknum):
        for source_index in range(sourcenum):
            time_address=np.digitize(time_components[(depth_slice[:,0]-1==source_index) & (depth_slice[:,1]-sourcenum-1==sink_index)],time_index)
            scatter_map[source_index,sink_index,0,time_address,idx]=np.sum(ray_components[(depth_slice[:,0]-1==source_index) & (depth_slice[:,1]-sourcenum-1==sink_index),0])
            scatter_map[source_index,sink_index,1,time_address,idx]=np.sum(ray_components[(depth_slice[:,0]-1==source_index) & (depth_slice[:,1]-sourcenum-1==sink_index),1])
            scatter_map[source_index,sink_index,2,time_address,idx]=np.sum(ray_components[(depth_slice[:,0]-1==source_index) & (depth_slice[:,1]-sourcenum-1==sink_index),2])
...
def BaseNetworkProcess(total_network,wavelength,scattering_index):
     ...
     for idx in range(scatterdepth):
         ...
         for idr in range(start_row,end_row+1):
             ...
             if idx==0:
                 path_distance=distance.euclidean(total_network[idr,np.asarray([0,1,2])+((0)*3)], total_network[idr,np.asarray([3,4,5])+((0)*3)])+path_distance
             else:
                 for idy in range(idx+1):
                     path_distance=distance.euclidean(total_network[idr,np.asarray([0,1,2])+((idy)*3)], total_network[idr,np.asarray([3,4,5])+((idy)*3)])+path_distance


             path_loss=((4*np.pi*path_distance)/wavelength)
             scatter_map[source_index,sink_index,idx]=(1/path_loss)*np.exp(-path_distance*wave_vector*1j)+scatter_map[source_index,sink_index,idx]
...
def rayHits1Dv2(ray_payload,point_indexing,sink_index):
    ...
    final_index=point_indexing[np.all(np.array([hitmap,np.isin(point_indexing[:,-1],sink_index)]),axis=0),:]
    filtered_index=point_indexing[np.all(np.array([hitmap,~np.isin(point_indexing[:,-1],sink_index)]),axis=0),:]

    return filtered_index,final_index

def launchRaycaster1Dv2(sources,sinks,scattering_points,io_indexing,environment_local):
    #cuda.select_device(0)
    start=timer()
    first_ray_payload=charge_rays_environment1Dv2(sources,sinks,scattering_points,io_indexing)
    prep_dt = timer() - start
    raystart=timer()
    ...
    filtered_index,final_index=rayHits1Dv2(first_ray_payload,io_indexing,sink_index)
    mem_dt = timer() - start
...
def launchRaycaster1Dv3(sources,sinks,scattering_points,io_indexing,environment_local):
    #cuda.select_device(0)
    start=timer()
    first_ray_payload=charge_rays_environment1Dv2(sources,sinks,scattering_points,io_indexing)
    prep_dt = timer() - start
...
def chunkingRaycaster1Dv2(sources,sinks,scattering_points,filtered_index,environment_local,terminate_flag):
    #cuda.select_device(0)
    start=timer()
    sink_index=np.arange(sources.shape[0]+1,sources.shape[0]+1+sinks.shape[0]).reshape(sinks.shape[0],1)
    ...
    second_ray_payload=charge_rays_environment1Dv2(sources,sinks,scattering_points,target_indexing)
    prep_dt = timer() - start
    raystart=timer()
   ...
   # cuda.close()
    ...
    start=timer()
    RAYS_CAST=ray_num
    filtered_index2,final_index2=rayHits1Dv2(second_ray_payload,target_indexing,sink_index)
    mem_dt = timer() - start
...
def chunkingRaycaster1Dv3(sources,sinks,scattering_points,filtered_index,environment_local,terminate_flag):
    start=timer()
    sink_index=np.arange(sources.shape[0]+1,sources.shape[0]+1+sinks.shape[0]).reshape(sinks.shape[0],1)
    scattering_point_index=np.arange(np.max(sink_index)+1,np.max(sink_index)+1+scattering_points.shape[0]).reshape(scattering_points.shape[0],1)
    if not terminate_flag:
    ...
    ray_limit = (np.floor(np.floor((max_mem - environment_local.nbytes) / base.ray_t.size) / 1e7) * 1e7).astype(int)
    if target_indexing.shape[0] >= ray_limit:
        # need to split the array and process seperatly
        sub_target = np.array_split(target_indexing, np.ceil(target_indexing.shape[0] / ray_limit).astype(int))
        ...
        for chunkindex in range(chunknum):
            ...
            threads_in_block = 256
            second_ray_payload = charge_rays_environment1Dv2(sources, sinks, scattering_points, sub_target[chunkindex])
            # ray_chunks=np.linspace(0,ray_num,math.ceil(ray_num/chunk_size)+1,dtype=np.int32)
            # for idx in range(len(triangle_chunk)-1):
            d_environment = cuda.device_array(len(environment_local), dtype=base.triangle_t)
            cuda.to_device(environment_local, to=d_environment)
            d_chunk_payload = cuda.device_array([second_ray_payload.shape[0]], dtype=base.ray_t)
            ...
            temp_filtered_index2, temp_final_index2 = rayHits1Dv2(second_ray_payload, sub_target[chunkindex], sink_index)
            mem_dt = timer() - start
...
def ray_charge_core_final_vector(temp_ray_payload,origins,source_sink_index,sources,targets,point_indexing,target_indexing):
    ...
    for n in range(len(source_chunking)-1):
        ...
        temp_ray_payload[source_chunking[n]:source_chunking[n+1]]['dx']=directions[:,0]
        temp_ray_payload[source_chunking[n]:source_chunking[n+1]]['dy']=directions[:,1]
        temp_ray_payload[source_chunking[n]:source_chunking[n+1]]['dz']=directions[:,2]
        ...
        temp_ray_payload[source_chunking[n]:source_chunking[n+1]]['dist']=norm_length[:,0]
        temp_ray_payload[source_chunking[n]:source_chunking[n+1]]['intersect']=False
        origins[source_chunking[n]:source_chunking[n+1],-origin_width:]=sources[n,:]
        source_sink_index[source_chunking[n]:source_chunking[n+1],0]=point_indexing[n,0]
        source_sink_index[source_chunking[n]:source_chunking[n+1],1]=target_indexing.ravel()
...
def shadowRaycaster(filtered_network,sinks,filtered_index,environment_local):
    ...
    second_ray_payload,second_origin,io_index2=charge_shadow_rays1D(filtered_network,sinks,filtered_index,target_indexing)
    prep_dt = timer() - start
    raystart=timer()
    ray_num=len(second_ray_payload)
    tri_num=len(environment_local)
    ...
    for idx in range(len(triangle_chunk)-1):
        ...
        for n in range(len(ray_chunks)-1):
            ...
            # Execute the kernel
            #cuda.profile_start()
            kernel1D[grids, threads](d_chunk_payload,d_environment,d_ray_flag,dist_list)
            #second_ray_payload[ray_chunks[n]:ray_chunks[n+1]]=d_chunk_payload.copy_to_host()
            second_ray_payload[ray_chunks[n]:ray_chunks[n+1]]['intersect']=d_ray_flag.copy_to_host()
    ...
    menstart=timer()
    RAYS_CAST=ray_num
    filtered_network2,final_network2,filtered_index2,final_index2=rayHits1D(second_ray_payload,io_index2,scatter_inc=1,origins_local=second_origin)
    mem_dt = timer() - menstart
    print("Stage: Prep {:3.1f} s, Raycasting  {:3.1f} s, Path Processing {:3.1f} s".format(prep_dt,kernel_dt,mem_dt) )
    return filtered_network2,final_network2,filtered_index2,final_index2,RAYS_CAST

def rayHits1D(ray_payload,point_indexing,scatter_inc=1,origins_local=None,initial_network=None):
    """
    filtering process for each raycasting stage, seprating rays which hit sinks from non-terminating rays.
    ----------
    ray_payload : TYPE
    ...
    """
    hitmap=~ray_payload[:]['intersect']
    filter_vector=np.ravel(np.nonzero(np.all(np.asarray([hitmap,np.isnan(point_indexing[:,1])]),axis=0)))
    output_vector=np.ravel(np.nonzero(np.all(np.asarray([hitmap,~np.isnan(point_indexing[:,1])]),axis=0)))
    #
    final_network=np.zeros((np.max(np.shape(output_vector)),3+(scatter_inc*3)),dtype=np.float32)
    ...
    #
    filtered_network=np.zeros((np.max(np.shape(filter_vector)),3+(scatter_inc*3)),dtype=np.float32)
...
def create_model_index(source_index,sink_index,scattering_point_index):
    ...
    """
    io_indexing=np.zeros((source_index.shape[0]*(sink_index.shape[0]+scattering_point_index.shape[0]),source_index.shape[1]+1),dtype=np.int32)
    idx=0
    for n in np.arange(0,source_index.shape[0]):
        if scattering_point_index.shape[0]==0:
            io_indexing[np.arange(idx,idx+(sink_index.shape[0]+scattering_point_index.shape[0])),-1]=sink_index.ravel()
        else:
            io_indexing[np.arange(idx,idx+(sink_index.shape[0]+scattering_point_index.shape[0])),-1]=np.append(sink_index,scattering_point_index,axis=0).ravel()

        io_indexing[np.arange(idx,idx+(sink_index.shape[0]+scattering_point_index.shape[0])),0:source_index.shape[1]]=source_index[n,:]
...
def workchunkingv2(sources,sinks,scattering_points,environment,max_scatter,line_of_sight=True):
    ...
    ray_limit=(np.floor(np.floor((max_mem-environment.nbytes)/base.ray_t.size)/1e7)*1e7).astype(int)
    #establish index boundaries
    source_index=np.arange(1,sources.shape[0]+1).reshape(sources.shape[0],1) # index starts at 1, leaving 0 as a null
    sink_index=np.arange(sources.shape[0]+1,sources.shape[0]+1+sinks.shape[0]).reshape(sinks.shape[0],1)
    scattering_point_index=np.arange(np.max(sink_index)+1,np.max(sink_index)+1+scattering_points.shape[0]).reshape(scattering_points.shape[0],1)
    #implement chunking
    if max_scatter==1:
        full_index=np.empty((0,2),dtype=np.int32)
        io_indexing = create_model_index(source_index,
                                         sink_index,
        ...
        if io_indexing.shape[0] >= ray_limit:
            ...
            for chunkindex in range(chunknum):
                _, temp_index, first_wave_Rays = launchRaycaster1Dv3(sources,
                                                                     sinks,
                                                                     scattering_points,
                                                                     sub_io[chunkindex],
        ...
        for chunkindex in range(source_chunking.size-1):
            ...
            if (filtered_index2.shape[0]*sinks.shape[0]>2e8):
                ...
                temp_chunking=np.linspace(0,filtered_index2.shape[0],temp_chunks,dtype=np.int32)
                temp_index=np.empty((0,4),dtype=np.int32)
                for temp_chunkindex in range(temp_chunking.shape[0]-1):
                    filtered_index3,final_index3_part,third_wave_rays=chunkingRaycaster1Dv3(sources,sinks,scattering_points,filtered_index2[temp_chunking[temp_chunkindex]:temp_chunking[temp_chunkindex+1],:],environment,True)
                    temp_index=np.append(temp_index,final_index3_part,axis=0)

                final_index3=temp_index
            else:
                filtered_index3,final_index3,third_wave_rays=chunkingRaycaster1Dv3(sources,sinks,scattering_points,filtered_index2,environment,True)
            ...
            temp_total=np.append(np.append(final_index,host_nans,axis=1),final_index2,axis=0)
            host_size2=np.shape(temp_total)
            host_nans2=np.zeros((host_size2[0],1),dtype=np.int32)
            full_index_temp=np.append(np.append(temp_total,host_nans2,axis=1),final_index3,axis=0)
...
def create_scatter_index(source_index,sink_index,scattering_point_index,scattering_mask):
    ...
    for launch_round in range(np.max(scattering_mask)+1):
        ...
        if launch_round==0:
            ...
            allowed_scatter_points=np.where(scattering_mask>=(launch_round+1))[0]+1
            temp_index=create_model_index(allowed_illuminators, sink_index,allowed_scatter_points.reshape(allowed_scatter_points.shape[0],1))
            target_index=np.append(target_index,np.append(temp_index,np.zeros((temp_index.shape[0],target_index.shape[1]-temp_index.shape[1]),dtype=np.int32),axis=1),axis=0)    
        else:
            allowed_illuminators=np.where(scattering_mask>=(launch_round+1))[0]+1
            #allowed_points=np.append(np.where(scattering_mask>=(launch_round+1))[0]+1,(np.where(scattering_mask==0)[0]+1))
            allowed_scatter_points=scattering_point_index
            propagate_index=np.where(target_index[:,launch_round]>np.max(sink_index))[0]
            drain_trim=np.delete(propagate_index,np.isin(target_index[propagate_index,launch_round],allowed_illuminators))
            drain_temp=create_model_index(target_index[drain_trim,0:launch_round+1],sink_index,np.empty((0)))
            illuminator_index=propagate_index[np.isin(target_index[propagate_index,launch_round],allowed_illuminators)]
            forward_temp=create_model_index(target_index[illuminator_index,0:launch_round+1],sink_index,allowed_scatter_points.reshape(allowed_scatter_points.shape[0],1))
            trimmed_index=np.delete(target_index,propagate_index,axis=0)
            target_index=np.append(trimmed_index,np.append(np.append(drain_temp,forward_temp,axis=0),np.zeros((drain_temp.shape[0]+forward_temp.shape[0],target_index.shape[1]-drain_temp.shape[1]),dtype=np.int32),axis=1),axis=0)
               
        #so far this 
        
    return target_index

def integratedraycastersetup(sources,sinks,scattering_points,environment,scattering_mask):
    #temp function to chunk the number of rays to prevent creation of ray arrays to large for memory
    raycasting_timestamp=timer()
    ...
    if max_scatter==0:
        ...
        ray_estimate=sources*(sinks+scattering_points.shape[0])+sources*scattering_points.shape[0]*sinks
    elif max_scatter==2:
        ray_estimate=sources*(sinks+scattering_points.shape[0])+sources*scattering_points.shape[0]*sinks+sources*scattering_points.shape[0]*(scattering_points.shape[0])*sinks
    #print("Total of {:3.1f} rays required".format(ray_estimate))
    #establish index boundaries
    source_index=np.arange(1,sources+1).reshape(sources,1) # index starts at 1, leaving 0 as a null 
    sink_index=np.arange(sources+1,sources+1+sinks).reshape(sinks,1)
    scattering_point_index=np.arange(np.max(sink_index)+1,np.max(sink_index)+1+(scattering_points.shape[0]-sources-sinks)).reshape((scattering_points.shape[0]-sources-sinks),1)
    #implement chunking
    ...
    for chunkindex in range(source_chunking.size-1):
        io_indexing=create_scatter_index(source_index[source_chunking[chunkindex]:source_chunking[chunkindex+1]],sink_index,scattering_point_index,scattering_mask)
        ...
        full_index=np.append(full_index,integratedRaycaster(io_indexing,scattering_points,environment),axis=0)
