...
def are_meshes_equal(mesh1, mesh2, rtol=1e-5):
    ...
    for i in range(len(mesh1.cells)):
        meshes_are_equal *= np.allclose(
            mesh1.cells[i].data, mesh2.cells[i].data, rtol=rtol
        )
    ## normals
    print("checking 'normffffals3")

    if (
        "nx" in mesh1.point_data
        and "ny" in mesh1.point_data
        and "nz" in mesh1.point_data
    ):
        if (
            "nx" in mesh2.point_data
            and "ny" in mesh2.point_data
            and "nz" in mesh2.point_data
        ):
            meshes_are_equal *= np.allclose(
                mesh1.point_data["nx"], mesh2.point_data["nx"], rtol=rtol
            )
            meshes_are_equal *= np.allclose(
                mesh1.point_data["ny"], mesh2.point_data["ny"], rtol=rtol
            )
            meshes_are_equal *= np.allclose(
                mesh1.point_data["nz"], mesh2.point_data["nz"], rtol=rtol
            )
        elif "Normals" in mesh2.point_data:
            meshes_are_equal *= np.allclose(
                mesh1.point_data["nx"], mesh2.point_data["Normals"][:, 0], rtol=rtol
            )
            meshes_are_equal *= np.allclose(
                mesh1.point_data["ny"], mesh2.point_data["Normals"][:, 1], rtol=rtol
            )
            meshes_are_equal *= np.allclose(
                mesh1.point_data["nz"], mesh2.point_data["Normals"][:, 2], rtol=rtol
            )
        else:
        ...
        if "nx" and "ny" and "nz" in mesh2.point_data:
            meshes_are_equal *= np.allclose(
                mesh1.point_data["Normals"][:, 0], mesh2.point_data["nx"], rtol=rtol
            )
            meshes_are_equal *= np.allclose(
                mesh1.point_data["Normals"][:, 1], mesh2.point_data["ny"], rtol=rtol
            )
            meshes_are_equal *= np.allclose(
                mesh1.point_data["Normals"][:, 2], mesh2.point_data["nz"], rtol=rtol
            )
            print("checking 'normals3")
        elif "Normals" in mesh2.point_data:
            meshes_are_equal *= np.allclose(
                mesh1.point_data["Normals"], mesh2.point_data["Normals"], rtol=rtol
            )
        else:
            meshes_are_equal *= False
    else:
    ...
    for i in range(len(mesh1.cells)):
        if "nx" and "ny" and "nz" in mesh1.cell_data:
            if "nx" and "ny" and "nz" in mesh2.cell_data:
                meshes_are_equal *= np.allclose(
                    mesh1.cell_data["nx"], mesh2.cell_data["nx"], rtol=rtol
                )
                meshes_are_equal *= np.allclose(
                    mesh1.cell_data["ny"], mesh2.cell_data["ny"], rtol=rtol
                )
                meshes_are_equal *= np.allclose(
                    mesh1.cell_data["nz"], mesh2.cell_data["nz"], rtol=rtol
                )
            elif "Normals" in mesh2.cell_data:
                meshes_are_equal *= np.allclose(
                    mesh1.cell_data["nx"], mesh2.cell_data["Normals"][:, 0], rtol=rtol
                )
                meshes_are_equal *= np.allclose(
                    mesh1.cell_data["ny"], mesh2.cell_data["Normals"][:, 1], rtol=rtol
                )
                meshes_are_equal *= np.allclose(
                    mesh1.cell_data["nz"], mesh2.cell_data["Normals"][:, 2], rtol=rtol
                )
            else:
                meshes_are_equal *= False
            ...
            if "nx" and "ny" and "nz" in mesh2.cell_data:
                meshes_are_equal *= np.allclose(
                    mesh1.cell_data["Normals"][:, 0], mesh2.cell_data["nx"], rtol=rtol
                )
                meshes_are_equal *= np.allclose(
                    mesh1.cell_data["Normals"][:, 1], mesh2.cell_data["ny"], rtol=rtol
                )
                meshes_are_equal *= np.allclose(
                    mesh1.cell_data["Normals"][:, 2], mesh2.cell_data["nz"], rtol=rtol
                )
            elif "Normals" in mesh2.cell_data:
                meshes_are_equal *= np.allclose(
                    mesh1.cell_data["Normals"], mesh2.cell_data["Normals"], rtol=rtol
                )
            else:
                meshes_are_equal *= False
        else:
...
def test_tri_centroids():
    ...
    refrenece_mesh = meshio.read(
        files(lyceanem.tests.data).joinpath("centroid_mesh.ply")
    )
    input = meshio.read(files(lyceanem.tests.data).joinpath("receive_horn.ply"))
...
def test_axes_from_normal_x_x():
    ...
    assert_allclose(
        GF.axes_from_normal(boresight, boresight_along="x"), rotation_matrix, atol=1e-12
    )
...
def test_axes_from_normal_y_x():
    ...
    rotation_matrix = R.from_euler("z", -90, degrees=True).as_matrix()
    assert_allclose(
        GF.axes_from_normal(boresight, boresight_along="y"), rotation_matrix, atol=1e-12
    )
...
def test_axes_from_normal_z_x():
    ...
    rotation_matrix = R.from_euler("y", 90, degrees=True).as_matrix()
    assert_allclose(
        GF.axes_from_normal(boresight, boresight_along="z"), rotation_matrix, atol=1e-12
    )
