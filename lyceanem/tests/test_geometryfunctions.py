...
def test_axes_from_normal_x_off():
    boresight = np.array([ 0.5, -0.5,  0.70710678])
    rotation_matrix = np.matmul(R.from_euler('y',45,degrees=True).as_matrix(),R.from_euler('z',45,degrees=True).as_matrix())
    assert_allclose(axes_from_normal(boresight,boresight_along='x'),rotation_matrix,atol=1e-9)
